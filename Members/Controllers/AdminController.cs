...
namespace Members.Controllers
{
    ...
    public class AdminController(IWebHostEnvironment environment, ILogger<AdminController> logger, ApplicationDbContext context /*, UserManager<IdentityUser> userManager, UserService userService*/) : Controller
    {
        ...
        // POST: Admin/DeleteCategoryConfirmed
        [HttpPost]
        ...
        [HttpPost("Admin/DeleteFileFromCategory/{id}/{categoryId}")]
        [ValidateAntiForgeryToken]
...
//using Members.Models; // Assuming your models are here
//using System.IO;
//using System.Linq;
//using System.Threading.Tasks;
//using Microsoft.EntityFrameworkCore;
...
//{
//    [Authorize(Roles = "Admin")]
//    public class AdminController(IWebHostEnvironment environment, ILogger<AdminController> logger, ApplicationDbContext context) : Controller
//    {
//        private readonly IWebHostEnvironment _environment = environment;
//        private readonly ILogger<AdminController> _logger = logger;
//        private readonly ApplicationDbContext _context = context;
...
//        // Action to display the main Manage Category Files view
//        public IActionResult ManageCategoryFiles(int? categoryId)
//        {
//            ViewBag.PDFCategories = _context.PDFCategories.OrderBy(c => c.SortOrder).ToList(); // Make sure you are loading your categories
//            ViewBag.SelectedCategoryId = categoryId;
//            ViewData["Title"] = "Manage Files in Category";

//            IEnumerable<CategoryFile> files = []; // Use List<CategoryFile> for easier manipulation

//            if (categoryId.HasValue)
//            {
//                // Retrieve files for the selected category from the database
//                var filesFromDb = _context.CategoryFiles
//                                            .Where(f => f.CategoryID == categoryId.Value)
//                                            .OrderBy(f => f.SortOrder) // Order by SortOrder for consistent listing
//                                            .ToList();

//                var filesToDelete = new List<CategoryFile>();
//                // Determine the path to your protected files folder
//                var protectedFilesPath = Path.Combine(_environment.ContentRootPath, "ProtectedFiles");

//                // Check if the protected files directory exists before proceeding
//                if (!Directory.Exists(protectedFilesPath))
//                {
//                    _logger.LogError("Protected files directory not found for cleanup: {Path}", protectedFilesPath);
//                    // Optionally, you might add a message to ViewBag to inform the user on the page
//                    // ViewBag.CleanupError = "Protected files directory not found. Automatic cleanup skipped.";
//                }
//                else
//                {
//                    foreach (var file in filesFromDb)
//                    {
//                        var filePath = Path.Combine(protectedFilesPath, file.FileName);
//                        if (!System.IO.File.Exists(filePath))
//                        {
//                            // File does not exist on disk, mark the database entry for deletion
//                            filesToDelete.Add(file);
...
//                {
//                    _context.CategoryFiles.RemoveRange(filesToDelete);
//                    // Use SaveChangesAsync for consistency, though SaveChanges is also fine here
...
//                // Re-fetch the list of files for the selected category *after* potential deletions.
//                // This ensures the view only displays files that have corresponding physical files and database entries.
//                // Use AsEnumerable() before Where to ensure the query is executed before the second Where (if needed for complex scenarios, otherwise directly querying _context is fine)
//                files = [.. _context.CategoryFiles
//                                  .Where(f => f.CategoryID == categoryId.Value)
...
//        [HttpPost]
//        [ValidateAntiForgeryToken]
//        public async Task<IActionResult> CreateCategory(PDFCategory category)
//        {
//            if (ModelState.IsValid)
//            {
//                _context.PDFCategories.Add(category);
...
//        {
//            var categories = _context.PDFCategories
//                .OrderBy(c => c.SortOrder)
//                .ThenBy(c => c.CategoryName)
//                .ToList();
...
//        public async Task<IActionResult> DeleteCategoryConfirmed(int id)
//        {
//            var category = await _context.PDFCategories.FindAsync(id);
//            if (category == null)
//            {
//                return NotFound();
//            }
...
//            if (ModelState.IsValid)
//            {
//                var categoryToUpdate = await _context.PDFCategories.FindAsync(categoryID);
...
//        public async Task<IActionResult> DeleteFileFromCategory(int id, int categoryId)
//        {
//            var categoryFileToDelete = await _context.CategoryFiles.FindAsync(id);
//            if (categoryFileToDelete != null)
//            {
//                var fileName = categoryFileToDelete.FileName; // Get the filename of the entry to be deleted
...
//                // Delete the database entry for the file in the current category first
//                _context.CategoryFiles.Remove(categoryFileToDelete);
//                await _context.SaveChangesAsync(); // Save changes to remove the entry
//                _logger.LogInformation("Deleted database entry for file ID {FileId} (FileName: {FileName}) from category {CategoryId}.", id, fileName, categoryId);

//                // Now, check if any other database entries still link to this physical file
//                var otherCategoryFiles = await _context.CategoryFiles
//                    .Where(cf => cf.FileName == fileName) // Find all entries with the same filename
//                    .ToListAsync(); // Execute the query to get the list
...
//                        catch (Exception ex)
//                        {
//                            _logger.LogError("Error deleting physical file {FileName} after last database entry deletion: {ErrorMessage}", filePath, ex.Message);
//                            // Optionally handle error (e.g., add a message to TempData to show on redirect)
//                        }
//                    }
//                    else
...
//        public async Task<IActionResult> RenameFileInCategory(int renameFileId, string oldFileName, string newFileName)
//        {
//            if (string.IsNullOrEmpty(newFileName) || oldFileName == newFileName)
//            {
//                return BadRequest("New file name cannot be empty or the same as the old file name.");
//            }
...
//            if (file != null && file.Length > 0)
//            {
//                var category = await _context.PDFCategories.FindAsync(categoryId);
//                if (category == null)
//                {
//                    return NotFound();
//                }
...
//                var filePath = Path.Combine(protectedFilesPath, finalFileName);
//                using (var fileStream = new FileStream(filePath, FileMode.Create))
//                {
//                    await file.CopyToAsync(fileStream);
//                }
...
//        [HttpGet]
//        public IActionResult GetNextSortOrder(int categoryId)
//        {
//            var existingFiles = _context.CategoryFiles
//                                          .Where(f => f.CategoryID == categoryId)
//                                          .OrderBy(f => f.SortOrder)
//                                          .ToList();

//            int nextSortOrder = 1; // Default`

//            if (existingFiles.Count != 0)
//            {
//                int highestSortOrder = existingFiles.Last().SortOrder;
...
//        public IActionResult CategoryFilesPartial(int categoryId)
//        {
//            var categoryFiles = _context.CategoryFiles
//                .Where(cf => cf.CategoryID == categoryId)
//                .ToList();
//            ViewBag.CategoryId = categoryId;
//            return PartialView(categoryFiles);
//        }
//    }
//}