...
[Authorize(Roles = "Admin")] // Ensure only Admin role can access this controller
public class PdfGenerationController : Controller // Inherit from Controller
{
    ...
    public PdfGenerationController(
        ...
        UserManager<IdentityUser> userManager, // Keep UserManager
    ...
    // Helper method to render a Razor view to a string (Keep this method if you use it elsewhere)    
    //private async Task<string> RenderViewToStringAsync<TModel>(string viewName, TModel model)
    //{
    //    _logger.LogInformation("RenderViewToStringAsync: Starting for view '{ViewName}'.", viewName);

    //    var actionContext = _actionContextAccessor.ActionContext;
    //    if (actionContext == null)
    //    {
    //        _logger.LogInformation("RenderViewToStringAsync: ActionContext is null, creating default.");
    ...
    //        );
    //        actionContext.HttpContext.RequestServices = HttpContext.RequestServices;
    //    }

    //    _logger.LogInformation("RenderViewToStringAsync: Finding view '{ViewName}'...", viewName);
    //    var viewResult = _razorViewEngine.FindView(actionContext, viewName, false);

    //    if (viewResult.View == null)
    //    {
    //        _logger.LogError("RenderViewToStringAsync: View '{ViewName}' not found. Searched locations: {Locations}", viewName, string.Join(", ", viewResult.SearchedLocations));
    //        throw new ArgumentNullException($"{viewName} view not found. Searched locations: {string.Join(", ", viewResult.SearchedLocations)}");
    //    }
    //    _logger.LogInformation("RenderViewToStringAsync: View '{ViewName}' found.", viewName);
    ...
    //    _logger.LogInformation("RenderViewToStringAsync: Rendering view '{ViewName}'...", viewName);
    //    await viewResult.View.RenderAsync(viewContext);
    //    _logger.LogInformation("RenderViewToStringAsync: View '{ViewName}' rendering complete.", viewName);

    //    var html = writer.ToString();
    //    _logger.LogInformation("RenderViewToStringAsync: Returning rendered HTML string.");
    //    return html;
    //}
    ...
    public async Task<IActionResult> CreatePdf() // Use async because we fetch data from DB
    {
        ...
        var viewModel = new CreatePdfFormViewModel
        {
            ...
            SuggestedSortOrder = initialSuggestedSortOrder // Pass the calculated suggested sort order
                                                           // The Categories list is no longer needed in this ViewModel as there's no dropdown
                                                           // Categories = null; // Ensure it's null or remove the property from ViewModel
        };
...
                // Bounds calculation using the reduced margin
                // This defines the main content area rectangle
                Syncfusion.Drawing.RectangleF bounds = new(margin, margin, page.GetClientSize().Width - (2 * margin), page.GetClientSize().Height - (2 * margin));
...
                // Initial currentY is set for drawing the main title with a reduced top margin
                float titleTopPosition = margin / 2; // Position title closer to the top edge
                float currentY = titleTopPosition; // Start currentY for drawing the title
                float currentX = bounds.Left;
                int currentColumn = 0;

                PdfFont titleFont = new PdfStandardFont(PdfFontFamily.Helvetica, 14, PdfFontStyle.Bold);
                float titleX = bounds.Left + (bounds.Width - titleFont.MeasureString("Oaks Village Homeowners Association Directory " + DateTime.Now.ToString("MM/yyyy")).Width) / 2;

                              // --- Corrected Image Loading ---
                // Use PdfSvgImage for SVG files
                PdfSvgImage logo = PdfSvgImage.FromFile("Images\\\\Svg\\\\Logos\\\\SmallLogo.svg");
                // --- End of Corrected Image Loading ---

                // Define image size
                float logoWidth = 20;
                float logoHeight = 20;
                // Draw the image
                graphics.DrawImage(logo, bounds.Left, currentY, logoWidth, logoHeight);


                // Draw the main title at the adjusted top position
...
                // Adjust currentY to start drawing content after the title with reduced space below it
                float spaceBelowTitle = margin / 2; // Reduced space below the title
                currentY += titleFont.MeasureString("Oaks Village Homeowners Association Directory " + DateTime.Now.ToString("MM/yyyy")).Height + spaceBelowTitle;
...
                            // Adjust currentY reset for new column on the same page
                            // Start drawing at the top of the content bounds after the initial title area
                            // Corrected MeasureString literal
                            currentY = titleTopPosition + titleFont.MeasureString("Oaks Village Homeowners Association Directory " + DateTime.Now.ToString("MM/yyyy")).Height + spaceBelowTitle; // Align with start of content in the first column
                        }
                        else
                        {
                            // New Page
...
                            float continuedTitleX = bounds.Left + (bounds.Width - titleFont.MeasureString("Homeowners Association Directory (Continued) " + DateTime.Now.ToString("MM/yyyy")).Width) / 2;
                            graphics.DrawString("Homeowners Association Directory (Continued) " + DateTime.Now.ToString("MM/yyyy"), titleFont, brush, new Syncfusion.Drawing.PointF(continuedTitleX, currentY)); // Draw continued title
...
                PdfFont footerFont = new PdfStandardFont(PdfFontFamily.Helvetica, 9);
                ...
                {
                    // Uncomment this line to right-justify the footer text
...
                using MemoryStream stream = new();
              ...
              // Fix for CS0103: The name 'PdfStringAlignment' does not exist in the current context
                // Replace 'PdfStringAlignment' with 'PdfTextAlignment' as per the provided type signatures.

                footerFormat.Alignment = PdfTextAlignment.Right;
            }
